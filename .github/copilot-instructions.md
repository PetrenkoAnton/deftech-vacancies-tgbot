...
Go-based Telegram bot with SQLite persistence that scrapes job vacancies from deftech.dou.ua and Dwarf Engineering sources. Single `Bot` struct in `app/bot.go` manages all operations: HTTP scraping, database interactions, and Telegram messaging.

## Key Components
- **main.go**: Environment loading, bot initialization, graceful shutdown
- **app/bot.go**: Core bot logic (1142 lines) - handlers, scraping, DB operations
...
- **External Sources**: Multiple Dwarf Engineering endpoints combined into single response
